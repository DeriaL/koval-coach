    ...
    <div>
      ...
      {upcoming.length > 0 && (
        <div className="mb-5">
          ...
          <div className="space-y-2">
            {upcoming.map(s => (
              <div key={s.id} className="card p-4 flex items-center gap-2 sm:gap-3">
                ...
                <button onClick={() => setCancelTarget(s)} disabled={pending} title="Скасувати з повідомленням клієнту"
                  className="btn text-sm px-2 hover:border-danger/40 hover:text-danger shrink-0"><Ban className="w-3.5 h-3.5" /></button>
                <button onClick={() => del(s.id)} disabled={pending} title="Видалити" className="btn text-sm px-2 text-muted shrink-0"><Trash2 className="w-3.5 h-3.5" /></button>
              </div>
            ))}
          </div>
        </div>
